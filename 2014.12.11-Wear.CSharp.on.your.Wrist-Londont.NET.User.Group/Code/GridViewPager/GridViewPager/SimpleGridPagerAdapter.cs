...
namespace GridViewPagerSample
{
	public class SimpleGridPagerAdapter : FragmentGridPagerAdapter
	{
		...
		public SimpleGridPagerAdapter (Context ctx, FragmentManager gm) :base (gm)
		{
			context = ctx;
		}
        ...
        readonly Page[][] Pages = new Page[][] {
            new Page [] { 
                new Page(Resource.String.welcome_title, Resource.String.welcome_text, Resource.Drawable.bugdroid, GravityFlags.CenterVertical) 
            },
            new Page [] { 
                new Page(Resource.String.about_title, Resource.String.about_text, false) 
            },
            new Page [] { 
		...
		{
			var page = Pages [row] [col];
			var title = page.TitleRes != 0 ? context.GetString (page.TitleRes) : null;
			var text = page.TextRes != 0 ? context.GetString (page.TextRes) : null;
			var fragment = CardFragment.Create (title, text, page.IconRes);
        ...
        public override Android.Graphics.Drawables.Drawable GetBackgroundForPage (int p0, int p1)
        {
            return context.Resources.GetDrawable (BgImages [RowCount % BgImages.Length]);
        }
